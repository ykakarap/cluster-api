...
func (c *DockerCluster) Default() {
	dockerclusterlog.Info("default", "name", c.Name)
	c.Spec.setDefaults()
}
...
func (c *DockerCluster) ValidateCreate() error {
	dockerclusterlog.Info("validate create", "name", c.Name)
	allErrs := c.Spec.validate()
	if len(allErrs) > 0 {
...
func (c *DockerCluster) ValidateUpdate(old runtime.Object) error {
	dockerclusterlog.Info("validate update", "name", c.Name)
	return nil
}

// ValidateDelete implements webhook.Validator so a webhook will be registered for the type.
func (c *DockerCluster) ValidateDelete() error {
	dockerclusterlog.Info("validate delete", "name", c.Name)
	return nil
}
