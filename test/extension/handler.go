...
import (
	...
	// FIXME(sbueringer): as long as catalog is internal it's very hard to write extensions.
...
type GroupVersionHookName struct {
	runtimecatalog.GroupVersionHook
	Name string
}
...
func (bld *HandlerBuilder) WithCatalog(c *runtimecatalog.Catalog) *HandlerBuilder {
	bld.catalog = c
	return bld
}

func (bld *HandlerBuilder) AddDiscovery(hook runtimecatalog.Hook, h Handler) *HandlerBuilder {
	return bld.AddExtension(hook, "", h)
}

func (bld *HandlerBuilder) AddExtension(hook runtimecatalog.Hook, name string, h Handler) *HandlerBuilder {
...
func (bld *HandlerBuilder) Build() (http.Handler, error) {
	...
	for g, h := range bld.hookToHandler {
		...
		r.HandleFunc(runtimecatalog.GVHToPath(gvhn.GroupVersionHook, gvhn.Name), fWrapper).Methods("POST")
	}
