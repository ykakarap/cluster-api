...
// Catalog contains mappings between Go types and the corresponding
// GroupVersionHooks, metadata and OpenAPIDefinitions.
type Catalog struct {
	// scheme allows access to api-machinery utilities to implement conversions of
	// request and response types.
	scheme *runtime.Scheme

	// gvhToType allows mapping a GroupVersionHook to the Go type.
	gvhToType map[GroupVersionHook]reflect.Type

	// gvhToType allows mapping a Go type to the GroupVersionHook.
	typeToGVH map[reflect.Type]GroupVersionHook

	// gvhToHookDescriptor allows mapping a GroupVersionHook to the hook descriptor.
	gvhToHookDescriptor map[GroupVersionHook]hookDescriptor

	// gvToOpenAPIDefinitions allows mapping a GroupVersion to a func
...
// hookDescriptor is an internal data structure which holds
...
func (c *Catalog) AddHook(gv schema.GroupVersion, hookFunc Hook, hookMeta *HookMeta) {
	...
	if t.NumIn() != 2 {
		panic("Hook must have two input parameter")
	}
	if t.NumOut() != 0 {
...
// GroupVersionHook returns the GVH of the Hook or an error if hook is not a pointer
// or not registered.
...
// TODO: discuss in which package this should be eventually placed.
