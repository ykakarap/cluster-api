...
func TestPatchNode(t *testing.T) {
	testCases := []struct {
		...
		// Add annotations (CAPI only enforce some annotation, never change or remove them)
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			g.Eventually(func(g Gomega) {
				...
				g.Expect(gotNode.Labels).To(Equal(tc.expectedLabels), cmp.Diff(gotNode.Labels, tc.expectedLabels))
				g.Expect(gotNode.Annotations).To(Equal(tc.expectedAnnotations), cmp.Diff(gotNode.Annotations, tc.expectedAnnotations))
				g.Expect(gotNode.Spec.Taints).To(Equal(tc.expectedTaints), cmp.Diff(gotNode.Spec.Taints, tc.expectedTaints))
